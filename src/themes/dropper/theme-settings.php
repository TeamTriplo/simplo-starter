...
/**
 * @file
 * Theme settings file for Dropper.
 *
 * Although Dropper itself does not provide any settings, we use this file to
...
if (module_exists('color')) {
  ...
  foreach ($fields as $field) {
    $form['header'][$field] = color_get_color_element('dropper', $field, $form);
  }
  ...
  foreach ($fields as $field) {
    $form['general'][$field] = color_get_color_element('dropper', $field, $form);
  }
  ...
  foreach ($fields as $field) {
    $form['primary_tabs'][$field] = color_get_color_element('dropper', $field, $form);
  }
  ...
  foreach ($fields as $field) {
    $form['footer'][$field] = color_get_color_element('dropper', $field, $form);
  }
} else {
  $form['color'] = array(
...
function dropper_form_system_theme_settings_alter(&$form, &$form_state) {
  $theme_name = $form['theme']['#value'];
  $dropper_path = backdrop_get_path('theme', 'dropper');

  backdrop_add_css($dropper_path . '/css/dropper-admin.css');
  backdrop_add_js($dropper_path . '/js/script.js');

  // @see _dropper_css_class()
  $form['settings']['bodyclass'] = array(
    '#type' => 'value',
    '#value' => '',
  );
  $form['#validate'][] = '_dropper_css_class';
 ...
 */
function _dropper_css_class($form, &$form_state) {
